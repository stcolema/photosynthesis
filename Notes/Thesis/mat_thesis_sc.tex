...
\par{\LARGE \textbf{Modelling photosynthetic parameters using mixed effects models and the Farquhar-van Cammerer-Berry model}}

\vspace{0.5cm}
MAT80418 \\
Thesis - Mathematical and Statistical Methods
...
A thesis of 18 credits presented for the degree of \\
...
Thanks are due to my supervisors, Gerrit, Elias and Rachel without whom this project would not have been possible. Jeremy Harbinson also offered guidance and advice which were appreciated - he pointed us towards the paper by \citet{YinUsingcombinedmeasurements2009} originally.
...
One of the driving factors in net assimilation rate ($A$) is the level of CO$_2$ present at the Rubisco site of carboxylation in the chloroplasts ($C_c$) \cite{VonCaemmererSteadystatemodelsphotosynthesis2013}. The intercellular CO$_2$ partial pressure ($C_i$) is measured directly. Of more interest is the estimation of $C_c$ \cite{YinTheoreticalreconsiderationswhen2009}.  We can relate $C_i$ to $C_c$ using Flick's first law relating the diffusive flux to the concentration  under steady-state assumptions. We have:
...
Much of the preceding concepts and theory make for pleasant reading, but we consider it useful to explicitly state our mixed-effects model and it's assumptions in one place.
...
\caption{LME model estimates of $R_d$ for treatment and side effects (model point estimate $\pm$ twice the model standard error).}
\label{fig:lme_rd}
\end{figure}
...
\caption{LME model estimates of $s$ for treatment and side effects (model point estimate $\pm$ twice the model standard error).}
\label{fig:lme_s}
\end{figure}
